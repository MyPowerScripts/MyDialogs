...
function Get-MyItemDialog()
{
  ...
  if ($PSBoundParameters.ContainsKey("Prompt"))
  {
    if ($ValueNameWidth)
    {
      $TempNameWidth = $ValueNameWidth
    }
    else
    {
      $TempNameWidth = $ValueName.Length + 2
    }
    ...
    $TempWidth = [Math]::Ceiling(($TempNameWidth + $ValueWidth) * $TempFont.Width)
  ...
  if ($PSBoundParameters.ContainsKey("ValueName"))
  {
    ...
    $MyDialogValueLabel.Size = New-Object -TypeName System.Drawing.Size(([Math]::Ceiling($TempNameWidth * $TempFont.Width)), $TempHeight)
  ...
  #>
    ...
    [RegEx]::Match($EventArg.KeyChar, $ValidateKeyPress)
  ...
  #>
    ...
    if (-not [String]::IsNullOrEmpty($MyDialogValueTextBox.Text))
    {
      ...
      {
        $MyDialogValueTextBox.Tag = $Match.Groups["Return"].Value
